...
```
CONTROLLER_SERVER_IP=$( source scripts/lib-cidr.sh 2>&1 >- ; echo $nodeIP)
```
...
- designate: fix designate-values.yaml
    replace bind9 cluster-ip in nameservers
...
```
  ...
  kubectl rollout restart  -n openstack deploy designate-worker
```
